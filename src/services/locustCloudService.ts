...
const getOrCreateLocustTerminal = async () => findLocustTerminal() || (await createTerminal());
...
export class LocustCloudService {
  ...
  constructor(private readonly ctx: vscode.ExtensionContext) {
    ...
    vscode.window.onDidEndTerminalShellExecution(event => {
      ...
      if (
        terminal.name === 'Locust' &&
        execution.commandLine.value.includes('locust') &&
        exitCode !== 0
      ) {
        vscode.window.showErrorMessage(
          'Failed to start locust command. Check the terminal for more details',
        );
      }
    });
  }
  ...
  async pickLocustfile(): Promise<string | undefined> {
    try {
      const uri = (await vscode.commands.executeCommand('locust.pickLocustfile')) as
        | vscode.Uri
        | undefined;
      return uri?.fsPath;
    } catch {
  ...
  private async runLocustHeadless(
    locustfileAbs?: string,
    extraArgs: string[] = [],
  ): Promise<boolean> {
  ...
  async runByTag() {
    const file = (await vscode.commands.executeCommand('locust.pickLocustfile')) as
      | vscode.Uri
      | undefined;
    if (!file) return;
